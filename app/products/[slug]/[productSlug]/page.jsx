...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        const res = await fetch(`https://siddhant-dhariwal-designs-e6u4.vercel.app/api/products/single/${productSlug}`);
        const data = await res.json();

        if (data) {
          setProduct(data);
          const initialImage = data.mainImage?.url || (data.gallery?.[0]?.url) || "/placeholder.jpg";
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center space-y-4">
      ...
      <p className="text-xs font-bold uppercase tracking-[0.3em] text-gray-400">Loading Excellence</p>
    </div>
  );
  ...
  if (!product) return (
    <div className="h-screen flex items-center justify-center">
      <div className="text-center">
        ...
        <Link href="/" className="text-sm font-bold border-b border-black pb-1">Return Home</Link>
      </div>
    </div>
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
        ...
        <div className="lg:col-span-5 flex flex-col justify-start pt-4">
          ...
          <div className="space-y-8 border-t border-gray-100 pt-8">
            <div>
              <h3 className="text-[11px] font-bold uppercase tracking-widest text-gray-900 mb-4">Description</h3>
              <p className="text-gray-500 leading-relaxed text-lg font-light">
                {product.description || "A masterclass in design and craftsmanship, specifically tailored for those who appreciate the finer details in life."}
              </p>
            </div>
