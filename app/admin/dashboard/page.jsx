...
import {
  ...
  ChevronRight, LayoutGrid, Settings, LogOut,
  Calendar, Globe, DollarSign
} from "lucide-react";

/**
 * ARCHITECT & SHOP ADMIN DASHBOARD (V2.5)
 * Support: Single Cover Image + Multiple Gallery Items (Images/Videos)
 */
...
export default function AdminDashboard() {
  ...
  const initialForm = {
    ...
    date: "",
    status: "published",
    image: null,      // Raw File for Main Cover
    gallery: [],      // Array of Raw Files for Gallery
  };

  const [form, setForm] = useState(initialForm);
  const [preview, setPreview] = useState(null); // Cover Preview URL
  ...
  const handleChange = (e) => {
    ...
    if (name === "image" && files?.[0]) {
      ...
      // Append new files to existing gallery files in state
  ...
  const handleEdit = (item) => {
    ...
    // Check for mainImage or legacy image field
    setPreview(item.mainImage?.url || item.image?.url || null);
    
    // Set existing gallery items
  ...
  const removeGalleryItem = (index, item) => {
    ...
    // Filter out the file from the raw gallery array
    setForm(prev => ({
      ...prev,
      gallery: prev.gallery.filter((_, i) => {
          // Calculate index relative to new files only
  ...
  const handleSubmit = async (e) => {
    ...
    // Single Cover Image -> Key: "mainImage" (Backend sync)
    if (form.image instanceof File) {
      formData.append("mainImage", form.image);
    }

    // Multiple Gallery Files -> Key: "gallery"
        ...
        <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-12">
          <div>
            <h2 className="text-4xl font-black text-slate-900 tracking-tight capitalize">
              {activeTab} <span className="text-indigo-600 text-5xl">.</span>
            </h2>
            <p className="text-slate-500 font-medium mt-1">Manage your storefront and architectural works.</p>
          </div>
          ...
          <div className="xl:col-span-7 space-y-6">
            {fetching ? (
                  ...
                  <div key={item._id} className="group bg-white rounded-[2rem] p-4 border border-white hover:border-indigo-100 hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-500">
                    <div className="relative h-56 w-full rounded-[1.5rem] overflow-hidden bg-slate-100 mb-5">
                      { (item.mainImage?.url || item.image?.url) ? (
                        <img 
                          src={item.mainImage?.url || item.image?.url} 
                      ...
                      <div className="absolute bottom-4 left-4">
                        <span className="px-3 py-1.5 bg-white/90 backdrop-blur rounded-lg text-[10px] font-black text-indigo-600 uppercase tracking-widest shadow-sm">
                          {item.category}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="px-2 pb-2">
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="font-bold text-slate-800 text-lg leading-tight line-clamp-1">{item.title}</h4>
                        {item.price && <span className="text-indigo-600 font-black text-lg">${item.price}</span>}
                      </div>
                      {item.location && (
              ...
              <div className="py-40 text-center bg-white rounded-[2rem] border-2 border-dashed border-slate-200">
                ...
                <h3 className="text-xl font-bold text-slate-400">No entries found</h3>
                <p className="text-slate-300 text-sm mt-1">Try changing your search or add a new item.</p>
              </div>
            )}
          </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-2xl shadow-slate-200/60 sticky top-8 overflow-hidden">
              <div className="p-8 bg-slate-900 text-white">
                <div className="flex justify-between items-center">
                  <div>
                    <h3 className="text-xl font-bold">{form._id ? "Edit Item" : "Create Entry"}</h3>
                    <p className="text-slate-400 text-[10px] uppercase font-black tracking-[0.2em] mt-1">Product & Portfolio Manager</p>
                  </div>
                  {form._id && (
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
                ...
                <div className="space-y-5">
                  <div className="group">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block ml-1">Item Title</label>
                    <input 
                      name="title" 
                      value={form.title} 
                      ...
                      className="w-full bg-slate-50 border-2 border-transparent p-4 rounded-2xl text-sm font-medium focus:bg-white focus:border-indigo-500 transition-all outline-none" 
                    />
                  </div>
                    ...
                    {activeTab === "products" ? (
                      <div className="group">
                        ...
                        <div className="relative">
                          ...
                          <input name="price" type="number" value={form.price} onChange={handleChange} className="w-full bg-slate-50 border-2 border-transparent pl-10 pr-4 py-4 rounded-2xl text-sm font-medium focus:bg-white focus:border-indigo-500 transition-all outline-none" />
                        </div>
                      </div>
                    ) : (
                      <div className="group">
                ...
                <div className="pt-4 border-t border-slate-100">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <span className="text-[10px] bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full font-black uppercase">
                      {galleryPreviews.length} Items
                    </span>
                  </div>
                ...
                <div className="pt-6">
                  <button
                    disabled={loading}
                    className="w-full py-5 bg-indigo-600 text-white rounded-3xl font-black text-xs uppercase tracking-[0.3em] shadow-2xl shadow-indigo-200 hover:bg-indigo-700 hover:-translate-y-1 active:translate-y-0 transition-all disabled:opacity-50"
                  >
                    {loading ? (
                    ...
                    ) : (form._id ? "Update Entry" : "Publish to Site")}
                  </button>
                </div>
              </form>
            </div>
